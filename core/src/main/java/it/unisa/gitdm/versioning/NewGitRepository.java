...
import static it.unisa.gitdm.source.NewGit.extractCommit;
 ...
 * @author Vincenzo
 */
public final class NewGitRepository implements CodeVersioningSystemRepository, Serializable {

    private File directory;
    private List<Commit> commits;
    private GitAuthorRepository authorRepository;

    public NewGitRepository() {
    ...
    public boolean initialize() {
        if (directory != null) {
            try {
                commits = extractCommit(directory, true);
                authorRepository = new GitAuthorRepository();
                authorRepository.init(this);
                return true;
            } catch (GitAPIException | IOException ex) {
                Logger.getLogger(NewGitRepository.class.getName()).log(Level.SEVERE, null, ex);
            }
    ...
    public Commit getCommitByID(String ID, boolean isSVN) {
        ...
        if (isSVN) {
            ...
            for (Commit c : commits) {
                System.out.println("Commit false:" + c);
    ...
    public void getSnapshot(Commit commit) throws GitAPIException, IOException {
        NewGit.gitCheckout(directory, commit);
